...
void _start(boot_info_t *boot_info_ptr)
{
    ...
    while (true)
    {
        ...
        while (mouse_ev.type != MOUSE_NO_TYPE)
        {
            ...
            {
                mouse_x = global_basic_renderer->frame_buffer->width - 8;
            }
            if (mouse_y > global_basic_renderer->frame_buffer->height)
            {
                mouse_y = global_basic_renderer->frame_buffer->height - 16;
            }
            ...
            if (mouse_ev.type == MOUSE_LEFT_BUTTON)
            {
                draw_char(boot_info_ptr->frame_buffer, boot_info_ptr->font, 0xFFFF0000, 'A', mouse_x, mouse_y);
            }
            draw_char(boot_info_ptr->frame_buffer, boot_info_ptr->font, 0, '@', old_mouse_x, old_mouse_y);
            draw_char(boot_info_ptr->frame_buffer, boot_info_ptr->font, 0xFFFFFFFF, '@', mouse_x, mouse_y);
            old_mouse_x = mouse_x;
            old_mouse_y = mouse_y;
